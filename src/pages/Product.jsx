  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `${URL}/get_product?search=${search}&page=${currentPage}&limit=${itemsPerPage}`,
        {
          method: "GET",
          headers: {
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-[#432DD7] text-white">
                <tr>
                  ...
                  <th className="px-4 py-2">Purches Price</th>
              ...
              <tbody>
                {product.length > 0 ? (
                  product.map((vendor, idx) => (
                    <tr key={vendor._id || vendor.id || idx} className="border-t border-gray-200">
                       <td className="px-4 py-2">{vendor?.adminId?.storeName}</td>
                      <td className="px-4 py-2">{vendor.productName}</td>
                      <td className="px-4 py-2">{vendor.quantity}</td>
                      <td className="px-4 py-2">{vendor.units}</td>
                      <td className="px-4 py-2">{vendor.purchesPrice}</td>
          ...
          {product.length > 0 && (
            <div className="flex justify-between items-center mt-4">
              <span className="text-sm text-gray-700 font-bold">
                Page {currentPage} of {totalPages || 1} (Total: {totalItems} product)
              </span>
